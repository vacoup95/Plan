...
public class DataSvc implements DataService {
    ...
    // TODO unfinished
    private static class TwoWayClassKeyMap<A, B, V> extends ArrayList<KeyValuePair<ClassPair<A, B>, V>> {
        public V get(A key) {
            for (KeyValuePair<ClassPair<A, B>, V> pair : this) {
                if (pair.key.a.equals(key)) return pair.value;
            }
            return null;
        }

        public V getReverse(B key) {
            for (KeyValuePair<ClassPair<A, B>, V> pair : this) {
                if (pair.key.b.equals(key)) return pair.value;
            }
            return null;
        }
    }
